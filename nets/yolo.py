...
class RepConv(nn.Module):
    ...
    def __init__(self,
        ...
        self.act = nn.LeakyReLU(0.1, inplace=True) if act is True \
            else (act if isinstance(act, nn.Module) else nn.Identity())
        if deploy:
            self.rbr_reparam = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=True)
        else:
            self.rbr_identity = (nn.BatchNorm2d(num_features=c1, eps=0.001,
                                                momentum=0.03) if c2 == c1 else None)
    ...
    def fuse_repvgg_block(self):
        ...
        self.rbr_1x1 = self.fuse_conv_bn(
            self.rbr_1x1[0],
            self.rbr_1x1[1]
        )
        ...
        if (
                isinstance(self.rbr_identity, nn.BatchNorm2d) \
                or \
                isinstance(self.rbr_identity, nn.modules.batchnorm.SyncBatchNorm)
        ):
            ...
            identity_conv_1x1.weight.data = (
                identity_conv_1x1.weight.data.to(
            ...
            identity_conv_1x1.weight.data.fill_diagonal_(0.0)
            identity_conv_1x1.weight.data = (
                identity_conv_1x1.weight.data.unsqueeze(2).unsqueeze(3))
            ...
            bias_identity_expanded = torch.nn.Parameter(
                torch.zeros_like(torch.zeros_like(rbr_1x1_bias))
            )
...
class YoloBody(nn.Module):
    def __init__(self,
        ...
        self.yolo_head_P5 = \
            nn.Conv2d(transition_channels * 32,
